...
public class InserisciProdotto extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Prodotto felpa=new Prodotto();
		felpa.setTipo("felpa");
		felpa.setPrezzo(15.00);
		felpa.setColore("nero");
		
		boolean isMultipart;
		String filePath;
		int maxFileSize=1000*1024;
		int maxMemSize=5*1024;
		filePath=getServletContext().getInitParameter("file_upload_prod");
		...
		factory.setRepository(new File("C:\\Users\\manuM\\git\\SIW\\AliveMusic\\WebContent\\img\\prodotti"));
		...
		try {
			...
			while (i.hasNext())
			{
				...
				if (!fi.isFormField())
				{
					...
					stringbuilder.setCharAt(12,'/');
					
					String path_da_memorizzare=String.valueOf(stringbuilder);
					
					felpa.setImmagine(path_da_memorizzare);
				...
				else
				{
					...
					{
						felpa.setNome(fi.getString());
					}
					
					else if (fieldname.equals("descrizione_p"))
					{
						felpa.setDescrizione(fi.getString());
					}
				}
		...
		u.aggiungiProdottoRealizzato(felpa);

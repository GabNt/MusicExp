...
public class ArtistaDaoJDBC implements ArtistaDao {
	...
	public void save(Artista art) {
		...
		try {
			String insert = "insert into artista(nome,on_tour,sito_web,immagine,genere,copertina) values (?,?,?,?,?,?)";
			PreparedStatement statement = connection.prepareStatement(insert);
			statement.setString(1, art.getNome());
			statement.setBoolean(2, art.isOn_tour());
			statement.setString(3, art.getSito_web());
			statement.setString(4, art.getImmagine_band());
			statement.setString(5, art.getGenere());
			statement.setString(6, art.getCopertina_band());
	...
	private void updateEventi(Artista art, Connection connection) throws SQLException{
		...
		for (Evento evento : art.getEventi_NoProxy()) {
			...
			if(result.next()){
				String update = "update evento SET artista = ? WHERE id = ?";
	...
	public void update(Artista art) {
		Connection connection = this.dataSource.getConnection();
		try {
			String update = "update artista SET on_tour = ?, sito_web = ?,immagine=? ,genere=?, copertina=? WHERE nome=?";
			PreparedStatement statement = connection.prepareStatement(update);
			statement.setBoolean(1, art.isOn_tour());
			statement.setString(2, art.getSito_web());
			statement.setString(3, art.getNome());
			statement.setString(4, art.getImmagine_band());
			statement.setString(5, art.getGenere());
			statement.setString(6, art.getCopertina_band());
			statement.executeUpdate();
	...
	public Artista findByPrimaryKey(String nome) {
		...
		try {
			...
			if (result.next()) {
				...
				artista.setOn_tour(result.getBoolean("on_tour"));

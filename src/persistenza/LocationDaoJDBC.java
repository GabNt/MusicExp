...
public class LocationDaoJDBC implements LocationDao {
	...
	public void save(Location loc) {
		...
		try {
			...
			statement.setDouble(4, loc.getLat());
			statement.setDouble(5, loc.getLng());
	...
	private void updateEventi(Location l,Connection c) throws SQLException
	{
		...
		for (Evento ev : l.getEventi_NoProxy()) {
	...
	public void update(Location loc) {
		...
		try {
			...
			statement.setDouble(3, loc.getLat());
			statement.setDouble(4, loc.getLng());
	...
	public Location findByPrimaryKey(String nome) {
		...
		try {
			...
			if (result.next()) {
				...
				location.setLat(result.getDouble("lat"));
				location.setLng(result.getDouble("lng"));
			}
		} catch (SQLException e) {
			throw new PersistenceException(e.getMessage());
		} finally {

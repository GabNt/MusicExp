...
function Experience(id, limiteAdesione, posti, descrizione)
{
	this.id = id;
	this.limiteAdesione = new  Date(limiteAdesione);
	this.posti = posti;
...
function dammiDettagliExperience(exp)
{
	...
	$.ajax({
		...
		success : function(experienceOrganizzatore)
		{
			...
			dataLimiteAdesione.val(experience.limite_adesione);
//			data del cazzo
...
//					if(postiInt >= 0)
//					{	
////					alert(experience.posti+"!="+Number(Number(postiInt)+Number(length)));
//					if( experience.posti != Number(Number(postiInt)+Number(length)) )
//					{
